...
export default class AddTask extends Component {
    ...
    save = () => {
        if (!this.state.desc.trim()) {
            Alert.alert('Atenção','Informe uma descrição para a tarefa.')
            return
        }
    ...
    render() {
        let datePicker = null
        if (Platform.OS === 'ios') {
            datePicker = <DatePickerIOS mode='date' date={this.state.date} onDateChange={date => this.setState({ date })}></DatePickerIOS>
        } else {
            datePicker = (
        ...
        return (
            <Modal onRequestClose={this.props.onCancel} visible={this.props.isVisible} animationType='slide' transparent={true} onShow={() => this.setState({ ...this.getInitialState() })}>
                ...
                <View style={styles.container}>
                    ...
                    <TextInput placeholder="Descrição da tarefa" style={styles.input} onChangeText={desc => this.setState({ desc })} value={this.state.desc}></TextInput>
                    {datePicker}
